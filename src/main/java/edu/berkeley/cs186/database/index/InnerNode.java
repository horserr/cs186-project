...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        return getSuitableChild(key).get(key);
    }
...
//        Optional<Pair<DataBox, Long>> popUp = get(key).put(key, rid);     fixed
        Optional<Pair<DataBox, Long>> popUp = getSuitableChild(key).put(key, rid);
        if (!popUp.isPresent()) {
            return Optional.empty();
        }
        DataBox popKey = popUp.get().getFirst();
        Long popPageNum = popUp.get().getSecond();

        int i = Collections.binarySearch(keys, popKey);
        assert i < 0;       // not found
        int index = -i - 1;
        int order = metadata.getOrder();
        boolean isFull = keys.size() == order * 2;

        List<DataBox> tmpKeys = new ArrayList<>(keys.subList(0, index));
        tmpKeys.add(popKey);
        tmpKeys.addAll(keys.subList(index, keys.size()));
        keys = tmpKeys;

        List<Long> tmpChildren = new ArrayList<>(children.subList(0, index + 1));  // mind here
        tmpChildren.add(popPageNum);
        tmpChildren.addAll(children.subList(index + 1, children.size()));
        children = tmpChildren;

        if (isFull) {
            List<DataBox> newKeys = new ArrayList<>(keys.subList(order + 1, keys.size()));          // mind here
            List<Long> newChildren = new ArrayList<>(children.subList(order + 1, children.size())); // mind here
            InnerNode newInnerNode = new InnerNode(metadata, bufferManager, newKeys, newChildren, treeContext);
            Optional<Pair<DataBox, Long>> returning = Optional.of(new Pair<>(
                    keys.get(order),
    ...
    public void remove(DataBox key) {
        getSuitableChild(key).remove(key);
        sync();     // don't forget
    }
    ...
    /**
     * if found, return the node related to this key, otherwise, return the first
     * greater or equal node
     */
    private BPlusNode getSuitableChild(DataBox key) {
        int i = Collections.binarySearch(keys, key);
//        return i >= 0 ? getChild(i) : getChild(-i - 1);       fixed
        return i >= 0 ? getChild(i + 1) : getChild(-i - 1);
    }
